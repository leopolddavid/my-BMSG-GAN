...
class MSG_GAN:
    ...
    def __init__(self, depth=7, latent_size=512, use_eql=True, use_ema=True, ema_decay=0.999, device=th.device("cpu")):
        """ constructor for the class """
        ...
        if device == th.device("cuda"):
            ...
            self.dis = Discriminator(depth, latent_size//4, use_eql=use_eql, gpu_parallelize=True).to(device)   # todo: check if dividing this latent_size by 4 for the discriminator works
        else:
            self.dis = Discriminator(depth, latent_size//4, use_eql=True).to(device)
