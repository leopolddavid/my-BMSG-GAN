    ...
    def __init__(self, depth=7, latent_size=512, use_eql=True):
        ...
        if self.use_eql:
            def to_rgb(in_channels):
                return _equalized_conv2d(in_channels, 1, (1, 1), bias=True)  # todo: modify if for 1 channel only (3 --> 1)
        else:
            def to_rgb(in_channels):
                return Conv2d(in_channels, 1, (1, 1), bias=True)  # todo: modify if for 1 channel only (3 --> 1)
...
class Discriminator(th.nn.Module):
    ...
    def __init__(self, depth=7, feature_size=512, add_noise=False, use_eql=True, gpu_parallelize=False):
        """
        ...
        if self.use_eql:
            def from_rgb(out_channels):
                return _equalized_conv2d(1, out_channels, (1, 1), bias=True)  # todo: modify if for 1 channel only (3 --> 1)
        else:
            def from_rgb(out_channels):
                return Conv2d(1, out_channels, (1, 1), bias=True)  # todo: modify if for 1 channel only (3 --> 1)
        ...
        self.final_block = DisFinalBlock(self.feature_size, add_noise=add_noise, use_eql=self.use_eql)
    ...
    def forward(self, inputs):
        ...
        for x, block, converter in zip(reversed(inputs[1:-1]),
...
class MSG_GAN:
    ...
    def __init__(self, depth=7, latent_size=512, use_eql=True, use_ema=True, ema_decay=0.999, add_noise=False,
        ...
        if device == th.device("cuda"):
            ...
            self.dis = Discriminator(depth, latent_size // 4, add_noise=add_noise, use_eql=use_eql,
                                     gpu_parallelize=True).to(
                device)  # todo: check if dividing this latent_size by 4 for the discriminator works
        else:
            self.dis = Discriminator(depth, latent_size // 4, add_noise=add_noise, use_eql=True).to(device)
    ...
    def train(self, data, gen_optim, dis_optim, loss_fn, normalize_latents=True,
              ...
              log_dir=None, sample_dir="./samples", save_dir="./models"):
        """
        ...
        for epoch in range(start, num_epochs + 1):
            ...
            for (i, batch) in enumerate(data, 1):
                ...
                if normalize_latents:
                    gan_input = (gan_input / gan_input.norm(dim=-1, keepdim=True) * (self.latent_size ** 0.5))

                # optimize the discriminator:
                dis_loss = self.optimize_discriminator(dis_optim, gan_input, images, loss_fn)

                # optimize the generator:
                gen_loss = self.optimize_generator(gen_optim, gan_input, images, loss_fn)
                ...
                if i % (int(limit / feedback_factor) + 1) == 0 or i == 1:  # Avoid div by 0 error on small training sets
                    ...
                    if log_dir is not None:
                        ...
                        with open(log_file, "a") as log:
                            log.write(str(global_step) + "\t" + str(dis_loss) + "\t" + str(gen_loss) + "\n")

                    if epoch % 5 == 0 and i == 1:       # todo: check if this works
            ...
            if epoch % checkpoint_factor == 0 or epoch == 1 or epoch == num_epochs:
                ...
                gen_optim_save_file = os.path.join(save_dir, "GAN_GEN_OPTIM_" + str(epoch) + ".pth")
                dis_optim_save_file = os.path.join(save_dir, "GAN_DIS_OPTIM_" + str(epoch) + ".pth")
                ...
                if self.use_ema:
                    gen_shadow_save_file = os.path.join(save_dir, "GAN_GEN_SHADOW_" + str(epoch) + ".pth")
