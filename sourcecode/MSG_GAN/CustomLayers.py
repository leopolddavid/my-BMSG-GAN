...
# my gaussian noise module:
class GaussianNoise(th.nn.Module):
    def __init__(self, stddev=1/20):
    ...
    def forward(self, x):
        ...
        return x + th.autograd.Variable(th.randn(x.size()) * self.stddev).cuda()
        # return x
...
class GenInitialBlock(th.nn.Module):
    ...
    def __init__(self, in_channels, use_eql=True):
        ...
        """
        from torch.nn import Conv2d, ConvTranspose2d, LeakyReLU, PReLU
        super().__init__()
        ...
        # self.lrelu = PReLU()      # todo: check if this brought improvements
...
class GenGeneralConvBlock(th.nn.Module):
    ...
    def forward(self, x):
        ...
        y = interpolate(x, scale_factor=2)      # y = th.nn.Upsample(scale_factor=2, mode='nearest')(x)
...
class DisFinalBlock(th.nn.Module):
    ...
    def __init__(self, in_channels, use_eql=True, add_noise=False):
        """
        ...
        if use_eql:
            ...
            self.conv_3 = _equalized_conv2d(in_channels, 1, (1, 1), bias=True)  # final layer emulates the fully connected layer

        else:
            # modules required:
            self.conv_1 = Conv2d(in_channels + 1, in_channels, (3, 3), padding=1, bias=True)
            self.conv_2 = Conv2d(in_channels, in_channels, (4, 4), bias=True)
            self.conv_3 = Conv2d(in_channels, 1, (1, 1), bias=True)  # final conv layer emulates a fully connected layer
    ...
    def forward(self, x):
        ...
        if self.add_noise:
            y = self.batch_discriminator(self.gaussian_noise(x))
        else:
...
class DisGeneralConvBlock(th.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, use_eql=True, add_noise=False):
        """
        constructor of the class
        :param in_channels: number of input channels
    ...
    def forward(self, x):
        ...
        # define the computations
        if self.add_noise:
            y = self.lrelu(self.conv_1(self.gaussian_noise(x)))
        else:
